...
const Shipping = () => {
    ...
    useEffect(() => {
        if (order && (statusPage === EStatusPage.EDIT || statusPage === EStatusPage.NEW)) {
            const ship_exist = ships.find(
                (shipItem: any) =>
                    shipItem?.serviceId === order.deliveryOptions?.serviceId &&
                    shipItem.transportType === order.deliveryOptions?.transportType
            );
